i have created in laravel ImageService to generate images with text but the response to show the generated image is slow i want to store the generated images in cache as files for 24 hours to optimize the loading and i want the cached files to be cleared after expire or if i cleared the cache manually using artisan below the codes of the service and routes and Controllers
...
Route::get('imgen/test', [QuoteImageController::class, 'test'])->name('imgen.test');
Route::get('imgen/file', [QuoteImageController::class, 'file'])->name('imgen.file')->withoutMiddleware(VerifyCsrfToken::class);
